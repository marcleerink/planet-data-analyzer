...
def plot_images_per_land_cover_class(gdf_land_cover: gpd.GeoDataFrame) -> st.plotly_chart:

    # create fig to plot on
    fig = go.Figure()

    for featureclass in gdf_land_cover['featureclass'].unique():
        
        _gdf = gdf_land_cover[gdf_land_cover['featureclass'] == featureclass]
        
        grouped_gdf = _gdf.groupby(['featureclass'])[['total_images']].sum()
        
        fig.add_trace(go.Bar(x=grouped_gdf.index, y=grouped_gdf['total_images'], name=featureclass))
